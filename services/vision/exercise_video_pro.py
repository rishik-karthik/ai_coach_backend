...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        options = vision.PoseLandmarkerOptions(
            base_options=base_option, #model
            running_mode=vision.RunningMode.VIDEO,
            min_pose_detection_confidence=0.7, #object(human) detection
            min_pose_presence_confidence=0.7, #joint/limb detection
            min_tracking_confidence=0.7, #ideo is just a fast series of pictures. Once the AI finds the skeleton in Frame 1, it uses "tracking" to predict where the skeleton will be in Frame 2, rather than starting the search all over again
            output_segmentation_masks=False #if true blurs background and focus on object
        )
